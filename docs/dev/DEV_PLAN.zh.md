...
> **第一阶段架构要求(2026-09-20 修订)**:每个提问节点保存**统一锚点**——`kind: pdf`(页码+文本选区+章节)或 `kind: content`(回答节点+选中片段);点击历史节点同时恢复问答上下文并按锚点类型定位(pdf→跳 PDF 页高亮 / content→聊天区滚动高亮片段);树结构不只依赖原文锚点,AI 对话内容同样可展开与回溯;阅读记录从第一天起采用**可导出的可移植格式**,不绑定 pi-tree 数据库。
...
### Phase 5 — 双类锚点闭环:划词/划章节提问 + 锚点回跳 + 阅读记录导出(2.5-3.5 天)
...
3. **统一锚点数据模型**(核心扩展,为 Zotero 第二阶段铺路):
   - 提问节点存储统一锚点 `anchor: { kind: "pdf"|"content", ... }`:
     - `pdf`: `{ page, quote, section }`(页码、选中原文、所在章节);
     - `content`: `{ nodeId, quote }`(回答节点、选中的回答片段);
   - **PDF 锚点**:P4 已采集;开分支时写入节点;
   - **内容锚点**:聊天区在 AI 回答上划词(SelectionToolbar 已接入 ChatView)→ 以该片段为锚点开分支并写入节点;回答渲染需稳定节点 id,支持回溯时滚动定位 + 片段高亮;
   - 树视图与历史节点点击 → **恢复该分支问答上下文 + 按锚点类型定位**:pdf 锚点 → PDF 面板跳 anchor.page 并高亮 quote;content 锚点 → 聊天区滚动到对应回答并高亮 quote;
4. **可移植阅读记录导出**:新增 `/api/sessions/:id/reading-record`(或客户端导出)产出与 Zotero 兼容的 JSON:
   ```json
   { "source": "paper.pdf",
     "nodes": [ { "id": "n1", "parentId": null,
                  "question": "…", "answer": "…",
                  "anchor": { "kind": "pdf", "page": 4, "quote": "…", "section": "3.2" } },
                { "id": "n2", "parentId": "n1",
                  "question": "…", "answer": "…",
                  "anchor": { "kind": "content", "nodeId": "n1", "quote": "…" } } ] }
   ```
   (Pi SDK 树 ↔ 平铺 nodes 数组双向可映射;Zotero 与 pi-tree 读同一份记录,无需迁移聊天历史);
5. 验证完整闭环:PDF 划词 → 提问(带 pdf 锚点)→ 回答内划词追问(带 content 锚点)→ InlineBranches 追问 → 面包屑/树导航返回上层 → 点击历史节点 → **按锚点类型定位(pdf 跳页高亮 / content 滚动高亮)**。
- **验收**:e2e 用例(划词提问 + 章节提问 + 回答内划词追问 + 返回 + 双类锚点定位 + 导出记录)全绿。
...
| P5 | 双类锚点闭环 + 回跳 + 阅读记录导出 | 2.5-3.5 天 | P2+P4 |
| P6 | 收尾发布 | 1 天 | 全部 |

**总计:主线约 12-16.5 人天,全部必做(无可选阶段)。** P1 / P2 / P3 可并行推进;P4 依赖 P3;P5 依赖 P4。
...
4. **内容锚点定位粒度**:整条消息 vs 消息内片段(建议片段:存 quote,渲染时按文本匹配高亮;稳定 nodeId 由消息渲染层提供)。
5. **可信源清单**:MCP 接哪些源(建议 Tavily/Brave 搜索 + arXiv/Semantic Scholar 学术库),是否允许百科(标二手)。开工 P2 前定。
6. **上游策略**:纯 fork 独立演进 vs fork+定期同步。建议后者。
7. **部署形态**:Docker / 源码 / Electron 桌面?影响 P6 验证面,不影响主体开发。
